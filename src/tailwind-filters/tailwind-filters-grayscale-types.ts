// noinspection JSUnusedGlobalSymbols

/**
 * @group Filters
 * @category Grayscale
 * @css
 * ```css
 * filter: grayscale(0);
 * ```
 * @description You can refer to the official [tailwind documentation](https://tailwindcss.com/docs/grayscale)
 * @description You can refer to the [typedoc](https://git-init-wesley.github.io/tailwindcss-types/docs/types/grayscale_0.html)
 * @since Tailwind v3.2.7
 */
export type grayscale_0 = 'grayscale-0'

/**
 * @group Filters
 * @category Grayscale
 * @css
 * ```css
 * filter: grayscale(100%);
 * ```
 * @description You can refer to the official [tailwind documentation](https://tailwindcss.com/docs/grayscale)
 * @description You can refer to the [typedoc](https://git-init-wesley.github.io/tailwindcss-types/docs/types/grayscale.html)
 * @since Tailwind v3.2.7
 */
export type grayscale = 'grayscale'

/**
 * @group Filters
 * @category Grayscale
 * @example
 * ```ts
 * let value: Grayscale //Only tailwindcss types
 * let value: string | Grayscale //Your custom tailwindcss with only tailwindcss type
 * ```
 * @description You can refer to the official [tailwind documentation](https://tailwindcss.com/docs/grayscale)
 * @description You can refer to the [typedoc](https://git-init-wesley.github.io/tailwindcss-types/docs/types/Grayscale.html)
 * @since Tailwind v3.2.7
 */
export type Grayscale =
    grayscale_0 | grayscale
